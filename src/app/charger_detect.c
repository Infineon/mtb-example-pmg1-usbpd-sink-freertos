...
cy_en_usbpd_status_t chgdet_init(cy_stc_pdstack_context_t *stack_ctx)
{
    ...
    if ((stack_ctx != NULL) && (stack_ctx->port < NO_OF_TYPEC_PORTS))
    {
        ...
        event_sema_handle[stack_ctx->port] = xSemaphoreCreateCounting(10,0);
        if(NULL == event_sema_handle[stack_ctx->port])
        {
            CY_ASSERT(0);
        }
    }
...
cy_en_usbpd_status_t chgdet_start(cy_stc_pdstack_context_t *stack_ctx)
{
    ...
    /* Set RTOS event */
    xSemaphoreGive(event_sema_handle[port]);
#else
    CY_UNUSED_PARAMETER(stack_ctx);
#endif /* (defined(CY_IP_MXUSBPD) || defined(CY_IP_M0S8USBPD)) */
...
bool chgdet_prepare_deepsleep(cy_stc_pdstack_context_t *stack_ctx)
{
    ...
    if((true != ret) && (port < NO_OF_TYPEC_PORTS))
    {
        xSemaphoreGive(event_sema_handle[port]);
    }
...
void chgdet_fsm_set_evt(cy_stc_pdstack_context_t *stack_ctx, uint32_t evt_mask)
{
    ...
    if(stack_ctx->port < NO_OF_TYPEC_PORTS)
    {
        if(is_in_isr() == true)
        {
            ...
            xSemaphoreGiveFromISR(event_sema_handle[stack_ctx->port], &xTaskWoken);
            portYIELD_FROM_ISR(xTaskWoken);
        }
        else
        {
            xSemaphoreGive(event_sema_handle[stack_ctx->port]);
        }
    }
}
...
bool chgdet_rtos_event_wait(cy_stc_pdstack_context_t *stack_ctx)
{
    if (stack_ctx->port >= NO_OF_TYPEC_PORTS)
    {
        return false;
    ...
    return (pdTRUE == xSemaphoreTake(event_sema_handle[stack_ctx->port], portMAX_DELAY));
}
