...
SemaphoreHandle_t fault_sema_handle[NO_OF_TYPEC_PORTS] = {NULL};
...
void app_conf_for_faulty_dev_removal(cy_stc_pdstack_context_t * context)
{
    ...
    if ((!dpm_stat->attach) || (dpm_stat->curPortRole == CY_PD_PRT_ROLE_SINK))
    {
        ...
        xSemaphoreGive(fault_sema_handle[port]);
    }
...
#if (VBUS_OVP_ENABLE || VBUS_UVP_ENABLE)
    fault_sema_handle[port] = xSemaphoreCreateCounting(10, 0);
    if(NULL == fault_sema_handle[port])
    {
        CY_ASSERT(0);
    }
#endif
...
#if (VBUS_OVP_ENABLE || VBUS_UVP_ENABLE)
    return (pdTRUE == xSemaphoreTake(fault_sema_handle[stack_ctx->port], portMAX_DELAY));
#else
    return false;
#endif
}
